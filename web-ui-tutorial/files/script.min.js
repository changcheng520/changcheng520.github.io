...
return function(angle){this._angle=angle;this._img.style[supportedCSS]="rotate("+(angle%360)+"deg)";}})()}})(jQuery);;(function($){$.fn.onePageNav=function(options){var opts=$.extend({},$.fn.onePageNav.defaults,options),onePageNav={};onePageNav.sections={};onePageNav.bindNav=function($el,$this,o){var $par=$el.parent(),newLoc=$el.attr('href'),$win=$(window);if(!$par.hasClass(o.currentClass)){if(o.begin){o.begin();}
onePageNav.adjustNav($this,$par,o.currentClass);$win.unbind('.onePageNav');$.scrollTo(newLoc,o.scrollSpeed,{easing:o.easing,offset:{top:-o.scrollOffset},onAfter:function(){if(o.changeHash){window.location.hash=newLoc;}
$win.bind('scroll.onePageNav',function(){onePageNav.scrollChange($this,o);});if(o.end){o.end();}}});}};onePageNav.adjustNav=function($this,$el,curClass){$this.find('.'+curClass).removeClass(curClass);$el.addClass(curClass);};onePageNav.getPositions=function($this,o){var $nav=$this.find('a');if(o.filter!==''){$nav=$nav.filter(o.filter);}
$nav.each(function(){var linkHref=$(this).attr('href'),divPos=$(linkHref).offset(),topPos=divPos.top;onePageNav.sections[linkHref.substr(1)]=Math.round(topPos)-o.scrollOffset;});};onePageNav.getSection=function(windowPos,o){var returnValue='',windowHeight=Math.round($(window).height()*o.scrollThreshold);for(var section in onePageNav.sections){if((onePageNav.sections[section]-windowHeight)<windowPos){returnValue=section;}}
return returnValue;};onePageNav.scrollChange=function($this,o){onePageNav.getPositions($this,o);var windowTop=$(window).scrollTop(),position=onePageNav.getSection(windowTop,o);if(position!==''){onePageNav.adjustNav($this,$this.find('a[href=#'+position+']').parent(),o.currentClass);}};onePageNav.init=function($this,o){var didScroll=false,$nav=$this.find('a');if(o.filter!==''){$nav=$nav.filter(o.filter);}
$nav.bind('click',function(e){onePageNav.bindNav($(this),$this,o);e.preventDefault();});onePageNav.getPositions($this,o);$(window).bind('scroll.onePageNav',function(){didScroll=true;});setInterval(function(){if(didScroll){didScroll=false;onePageNav.scrollChange($this,o);}},250);};return this.each(function(){var $this=$(this),o=$.meta?$.extend({},opts,$this.data()):opts;onePageNav.init($this,o);});};$.fn.onePageNav.defaults={currentClass:'current',changeHash:false,easing:'swing',filter:'',scrollSpeed:750,scrollOffset:0,scrollThreshold:0.5,begin:false,end:false};})(jQuery);(function($){var $window=$(window);var windowHeight=$window.height();$window.resize(function(){windowHeight=$window.height();});$.fn.parallax=function(xpos,speedFactor,outerHeight){var $this=$(this);var getHeight;var firstTop;var paddingTop=0;$this.each(function(){firstTop=$this.offset().top;});if(outerHeight){getHeight=function(jqo){return jqo.outerHeight(true);};}else{getHeight=function(jqo){return jqo.height();};}
if(arguments.length<1||xpos===null)xpos="50%";if(arguments.length<2||speedFactor===null)speedFactor=0.1;if(arguments.length<3||outerHeight===null)outerHeight=true;function update(){var pos=$window.scrollTop();$this.each(function(){var $element=$(this);var top=$element.offset().top;var height=getHeight($element);if(top+height<pos||top>pos+windowHeight){return;}
$this.css('backgroundPosition',xpos+" "+Math.round((firstTop-pos)*speedFactor)+"px");});}
$window.bind('scroll',update).resize(update);update();};})(jQuery);(function($){$._spritely={animate:function(options){var el=$(options.el);var el_id=el.attr('id');if(!$._spritely.instances[el_id]){return this;}
...
return this;}})})(jQuery);try{document.execCommand("BackgroundImageCache",false,true);}catch(err){};(function($){$.fn.DS_scrollAnim=function(options){var settings=$.extend({"loadOkEvent":function(){},"backEvent":function(){},"author":"xizon | https://www.c945.com","version":"1.0","marginRight":0,"scrollWarpper":$(window),"marginLeft":0,"marginTop":0,"speed":1500,"easing":"swing","targetScrollDistance":0,"AngleInit":0,"AngleAnimateTo":0,"AngleSpeed":2000,"AngleEasing":false,"width":"default","height":"default","opacityInit":1,"opacityAnimateTo":1,"scrollType":"vertical"},options);this.each(function(){var $this=$(this),$marginRight=settings.marginRight,$marginLeft=settings.marginLeft,$marginTop=settings.marginTop,$speed=settings.speed,$easing=settings.easing,$targetScrollDistance=settings.targetScrollDistance,$AngleInit=settings.AngleInit,$AngleAnimateTo=settings.AngleAnimateTo,$AngleSpeed=settings.AngleSpeed,$AngleEasing=settings.AngleEasing,$width=settings.width,$height=settings.height,$opacityInit=settings.opacityInit,$opacityAnimateTo=settings.opacityAnimateTo,$scrollType=settings.scrollType,$scrollWarpper=settings.scrollWarpper;var chkAnim=false;var chkAnimback=false;var rotateEasing;var DBody=$(document).height()-100,DWidth=$this.css("width"),DHeight=$this.css("height"),DMarginTop=$this.css("margin-top"),DMarginLeft=$this.css("margin-left"),DMarginRight=$this.css("margin-right");var ChkDis;if($width=="default"){$width=DWidth};if($height=="default"){$height=DHeight};if($AngleEasing==false){$AngleEasing=$.easing.swing};if(chkAnim==false){if($AngleInit>0)$this.rotate($AngleInit);if($opacityInit>0)$this.css({opacity:$opacityInit});}
$scrollWarpper.scroll(function(){if($scrollType=="vertical"){ChkDis=$scrollWarpper.scrollTop();}else{ChkDis=$scrollWarpper.scrollLeft();}
if(ChkDis>$targetScrollDistance){if(chkAnim==false){if($marginTop!=0&&$marginRight==0&&$marginLeft==0){$this.animate({marginTop:$marginTop+"px",width:$width+"px",height:$height+"px",opacity:$opacityAnimateTo},{duration:$speed,easing:$easing});}
if($marginTop!=0&&$marginRight!=0&&$marginLeft==0){$this.animate({marginTop:$marginTop+"px",marginRight:$marginRight+"px",width:$width+"px",height:$height+"px",opacity:$opacityAnimateTo},{duration:$speed,easing:$easing});}
if($marginTop!=0&&$marginRight==0&&$marginLeft!=0){$this.animate({marginTop:$marginTop+"px",marginLeft:$marginLeft+"px",width:$width+"px",height:$height+"px",opacity:$opacityAnimateTo},{duration:$speed,easing:$easing});}
if($marginTop==0&&$marginRight!=0){$this.animate({marginRight:$marginRight+"px",width:$width+"px",height:$height+"px",opacity:$opacityAnimateTo},{duration:$speed,easing:$easing});}
if($marginTop==0&&$marginLeft!=0){$this.animate({marginLeft:$marginLeft+"px",width:$width+"px",height:$height+"px",opacity:$opacityAnimateTo},{duration:$speed,easing:$easing});}
$this.rotate({duration:$AngleSpeed,angle:$AngleInit,animateTo:$AngleAnimateTo,easing:$AngleEasing});settings.loadOkEvent();chkAnimback=true;}
chkAnim=true;}
if(ChkDis<=$targetScrollDistance){if(chkAnimback==true){settings.backEvent();}
chkAnimback=false;chkAnim=false;}});})}})(jQuery);if(!window.console){console=(function(){var instance=null;function Constructor(){this.div=document.createElement("console");this.div.id="console";this.div.style.cssText="filter:alpha(opacity=80);position:absolute;top:0px;left:0px;width:100%;border:1px solid #ccc;background:#eee;";document.body.appendChild(this.div);}
Constructor.prototype={log:function(str){var tp=document.createElement("p");tp.setAttribute("style","display:none");tp.innerHTML=str;this.div.appendChild(tp);}}
function getInstance(){if(instance==null){instance=new Constructor();}
return instance;}
return getInstance();})()}
$(function(){$("#nav").onePageNav({begin:function(){console.log("start")},end:function(){console.log("stop")},scrollOffset:0}),$("#nav a").on("click",function(){var a=$(this).attr("href");$("html, body").animate({scrollTop:$(a).offset().top},1e3)}),$("a.go").click(function(a){$(".ripple").remove();var b=$(this).offset().left,c=$(this).offset().top,d=$(this).width(),e=$(this).height();$(this).prepend("<span class='ripple'></span>"),d>=e?e=d:d=e;var f=a.pageX-b-d/2,g=a.pageY-c-e/2;$(".ripple").css({width:d,height:e,top:g+"px",left:f+"px"}).addClass("rippleEffect")}),$(".scene-2-rocket-box").parallax("50%",.8),$(".scene-2-rocket-fire").parallax("50%",.8);var a=!0;$(".scene-2-rocket-box,.scene-2-rocket-fire").addClass("sceneEffect-rotate-default"),$(".scene-2-logo").DS_scrollAnim({loadOkEvent:function(b){$(".scene-2-rocket-box,.scene-2-rocket-fire").addClass("sceneEffect-rotate-loaded"),1==a&&($("#scene-cloud-1-2").pan({fps:30,speed:8.7,dir:"down",depth:10}),$("#scene-cloud-2-2").pan({fps:30,speed:3.2,dir:"down",depth:12}),$("#scene-cloud-3-2").pan({fps:30,speed:22.4,dir:"down",depth:13}),$("#scene-cloud-4-2").pan({fps:30,speed:11.2,dir:"down",depth:10}),$("#scene-cloud-5-2").pan({fps:30,speed:2.2,dir:"down",depth:10}),a=!1)},targetScrollDistance:document.documentElement.clientHeight/2}),$(".scene-3-ele3").DS_scrollAnim({loadOkEvent:function(a){},targetScrollDistance:document.documentElement.clientHeight+document.documentElement.clientHeight/2})}),$(document).ready(function(){function a(){$(".scene-arrow").animate({bottom:"+=10"},500).delay(500).animate({bottom:"-=10"},500,function(){setTimeout(a,500)})}$(".logo").hover(function(){},function(){}),$("#scene-cloud-1").pan({fps:30,speed:.7,dir:"left",depth:10}),$("#scene-cloud-2").pan({fps:30,speed:3.2,dir:"left",depth:12}),$("#scene-cloud-3").pan({fps:30,speed:.4,dir:"left",depth:13}),$("#scene-cloud-4").pan({fps:30,speed:1.2,dir:"left",depth:10}),a(),$.extend($.easing,{easeOutExpo:function(a,b,c,d,e){return b==e?c+d:d*(1-Math.pow(2,-10*b/e))+c},easeOutBounce:function(a,b,c,d,e){return(b/=e)<1/2.75?d*(7.5625*b*b)+c:b<2/2.75?d*(7.5625*(b-=1.5/2.75)*b+.75)+c:b<2.5/2.75?d*(7.5625*(b-=2.25/2.75)*b+.9375)+c:d*(7.5625*(b-=2.625/2.75)*b+.984375)+c}})});
