...
/* followWin 1.0  Designed: https://www.c945.com 没位道 */
/*
Use:
...
//jQuery.DS_inputCursorFocus
(function($){$.fn.DS_inputCursorFocus=function(options){var settings=$.extend({"targetBox":".blur"},options);return this.each(function(){var $this=$(this),$targetBox=settings.targetBox;var inputCursorFocus=(function(){var init=function(){$this.each(function(){var thisVal=$(this).val();if(thisVal!=""){$(this).siblings($targetBox).hide()}else{$(this).siblings($targetBox).show()};$(this).focus(function(){$(this).siblings($targetBox).hide()}).blur(function(){var val=$(this).val();if(val!=""){$(this).siblings($targetBox).hide()}else{$(this).siblings($targetBox).show()}})})};return{init:init}})();inputCursorFocus.init()})}})(jQuery);

/*Designed: https://www.c945.com 没位道*/
...
//jquery.DS_marquee
(function($){$.fn.DS_marquee=function(options){var settings=$.extend({"scrollLabel":"li:first","loopTime":2000,"speed":600},options);this.each(function(){var $this=$(this),$loopTime=settings.loopTime,$speed=settings.speed,$scrollLabel=settings.scrollLabel;var _wrap=$this;var _interval=$loopTime;var _moving;_wrap.hover(function(){clearInterval(_moving);},function(){_moving=setInterval(function(){var _field=_wrap.find($scrollLabel);var _h=_field.height();_field.animate({marginTop:-_h+'px'},$speed,function(){_field.css('marginTop',0).appendTo(_wrap);})},_interval)}).trigger('mouseleave');})}})(jQuery);



//去除IE下点击虚线边框
if ($.browser.msie) {$("body").find("a").each(function() {$(this).attr("hidefocus","true");});}
