...
var MyrootPath_Temp="/demo/d-s-skin-demo/_theme17/"; //网站绝对路径，安装时系统自动修改，如果自己更换，请填写您安装程序的根目录，比如  /blog/ 
...
$(document).ready(function(){  
	...
	$('.new-blog-list .rp_box img,.article-post img,.post-content img').each(function() {
	...
	//--------------------主导航& 二级导航
		...
		nav.find("li[data-slider='1']").each(function() {
			...
			if ($(this).find("ul").length > 0) {
				$("<span>").html("<i class='nav-trigon'>▲</i>").appendTo($(this).children(":first"));
			}
			...
			if ($(this).find("ul").length > 0) {
				...
				$(this).mouseenter(function() {
					...
					$(this).find(".nav-trigon").stop(true, true).slideDown(100);
				});
				
				//hide submenus on exit
				$(this).mouseleave(function() {
					$(this).find("ul").stop(true, true).slideUp(100);
					$(this).find(".nav-trigon").stop(true, true).slideUp(100);
				});
			}
		});
